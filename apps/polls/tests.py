...
def create_poll(question, days):
	...
	'days' offser to now (negative for polls published in the past,
...
class PollViewTest(TestCase):
	def test_index_view_with_no_polls(self):
			"""
			If no poll exist, an appropriate messgae should be displayed
			"""	
	...
	def test_index_view_with_future_poll_and_past_poll():
		...
		create_poll(question="Past poll.", days= -30)
		create_poll(question="Future_poll", days= 30)
		response = self.client.get(reverse('polls:index'))
		self.assertQuerysetEqual(
			response.context['latest_poll_list'],
			['<Poll: Past poll.>']
